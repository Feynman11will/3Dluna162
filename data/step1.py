...
def step1_python(case_path):
    ...
    :return:
    """
    case_pixels, numpyOrigin, spacing, isflip = load_itk_image(case_path)
    ...
    while flag == 0 and cut_num < bw.shape[0]:
        ...
        print(cut_num, bw.shape[0])

    bw = fill_hole(bw)
    bw1, bw2, bw = two_lung_only(bw, spacing)
    return case_pixels, bw1, bw2, spacing, numpyOrigin, isflip


def nstep1_python(case_path):
    """
    z轴方向上的切片过程
    :param case_path:
    :return:
    ...
    case, numpyOrigin, numpySpacing, isflip = load_itk_image(case_path)
    case_pixels, spacing = get_pixels_hu(case)
    bw = binarize_per_slice(case_pixels, spacing)
    flag = 0
    cut_num = 0
