...
@LastEditTime: 2019-11-01 14:38:49
@LastEditors: Please set LastEditors
@Description: In User Settings Edit
@FilePath: /3D-Lung-nodules-detection-master/training/classifier/data_classifier.py
'''
import numpy as np
import torch
...
class DataBowl3Classifier(Dataset):
    ...
    def __getitem__(self, idx, split=None):
        ...
        return 
            ...
            3. isnodlist 检测器中是否有目标 shape is topk
            4. y ct图像对应的样本的分类
        '''
        img = np.load(self.filenames[idx])  # [1,]
        if self.random_sample and self.phase == 'train':
            chosenid = sample(conf_list, topk, T=T)
            # chosenid = conf_list.argsort()[::-1][:topk]
        else:
            chosenid = conf_list.argsort()[::-1][:topk]
            # croplist shape [topk, 1, 96, 96, 96] coordlist shape = [topk, 3, 24, 24, 24]
        ...
        for i, id in enumerate(chosenid):
            ...
            coordlist[i] = coord
            isnodlist[i] = isnod
            # croplist shape [topk, 1, 96, 96, 96] coordlist shape = [topk, 3, 24, 24, 24] isnodlist=[topk]
        if self.phase != 'test':
