...
@LastEditTime: 2019-10-25 15:19:42
@LastEditors: your name
@Description: In User Settings Edit
@FilePath: /3D-Lung-nodules-detection-master/training/detector/main.py
'''
...
sys.path.append('../../')
sys.path.append('../')
from lib.split_combine import SplitComb

import detector.data_set as data_set
...
from net.layers import acc


parser = argparse.ArgumentParser(description='PyTorch DataBowl3 Detector')
parser.add_argument('--model', '-m', metavar='MODEL', default='net.res18',
                    help='model')
...
parser.add_argument('--save-dir', default='../../results/weights/', type=str, metavar='SAVE',
...
def test_():
    print(sys.path)
    model = import_module('net.res18')
    print(model)


def main():
    # --model res18 -b 4 --epochs $eps --save-dir ./results/res18
    global args
    ...
    if args.test == 1:
        ...
        split_comber = SplitComb(sidelen, config['max_stride'], config['max_stride'], margin, config['pad_value'])

        dataset_test = data_set.DataBowl3Detedctor(
            datadir,
            config_training['val_list'],
            config,
            phase='test',
    ...
    dataset_val = data_set.DataBowl3Detector(
        datadir,
        config_training['val_list'],
        config,
        phase = 'val')
    ...
    def get_lr(epoch):
        if epoch <= args.epochs * 0.5:
            ...
            lr = 0.01 * args.lr
        return lr
    pbar = tqdm(range(start_epoch, args.epochs + 1))
    for epoch in pbar:
        pbar.set_description('%3dth epochs'%epoch)
...
def train(data_loader, net, loss, epoch, optimizer, get_lr, save_freq, save_dir):
    ...
    for i, (data, target, coord) in enumerate(pbar2):
        # print('%dth '%i)
        ...
        pbar2.set_description('loss is %10.10f'%(loss_output[0]))
        # print('loss is :',loss_output[0])
